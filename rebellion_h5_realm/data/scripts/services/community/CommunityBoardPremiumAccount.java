	...
	public String getAction(Player player, String bypass)
	{
		...
		else if(bypass.startsWith("show_"))
		{
			...
			if (premium != null) {
				...
				html.replace("{drop_n}", String.valueOf(player.getRateItems()));
				html.replace("{drop_chance_n}", "x");
				html.replace("{drop_amount_n}", "x");
				html.replace("{spoil_n}", String.valueOf(player.getRateSpoil()));
				html.replace("{spoil_chance_n}", "x");
				html.replace("{spoil_amount_n}", "x");
				html.replace("{adena_n}", String.valueOf(player.getRateAdena()));
				html.replace("{weight_n}", String.valueOf(player.getWeightPercents()));
				html.replace("{craft_n}", "x");
				html.replace("{m_craft_n}", "x");
				html.replace("{extract_n}", "x");
				html.replace("{manor_n}", String.valueOf(Config.RATE_MANOR));
				html.replace("{quest_n}", String.valueOf(Config.RATE_QUESTS_DROP));
				html.replace("{quest_reward_n}", String.valueOf(Config.RATE_QUESTS_REWARD));
				html.replace("{pet_xp_n}", "x");
				html.replace("{raid_drop_chance_n}", String.valueOf(Config.RATE_DROP_RAIDBOSS));
				html.replace("{raid_drop_amount_n}", "x");
				html.replace("{herb_drop_chance_n}", String.valueOf(Config.RATE_DROP_HERBS));
				html.replace("{herb_drop_amount_n}", "x");
				// Have
				html.replace("{xp_h}", String.valueOf(player.getRateExp() + premium.getBonusExpRate()));
				html.replace("{sp_h}", String.valueOf(player.getRateSp() + premium.getBonusSpRate()));
				html.replace("{drop_h}", String.valueOf(player.getRateItems() + premium.getBonusDropRate()));
				html.replace("{drop_chance_h}", String.valueOf(premium.getBonusDropChance()));
				html.replace("{drop_amount_h}", String.valueOf(premium.getBonusDropAmount()));
				html.replace("{spoil_h}", String.valueOf(premium.getBonusSpoilRate()));
				html.replace("{spoil_chance_h}", String.valueOf(premium.getBonusSpoilChance()));
				html.replace("{spoil_amount_h}", String.valueOf(premium.getBonusSpoilAmount()));
				html.replace("{adena_h}", String.valueOf(premium.getBonusAdenaDropRate()));
				html.replace("{weight_h}", String.valueOf(premium.getBonusWeightLimitRate()));
				html.replace("{craft_h}", String.valueOf(premium.getBonusCraftChance()));
				html.replace("{m_craft_h}", String.valueOf(premium.getBonusMasterCraftChance()));
				html.replace("{extract_h}", String.valueOf(premium.getBonusExtractableRate()));
				html.replace("{manor_h}", String.valueOf(premium.getBonusManorDropRate()));
				html.replace("{quest_h}", String.valueOf(premium.getBonusQuestDropRate()));
				html.replace("{quest_reward_h}", String.valueOf(premium.getBonusQuestRewardRate()));
				html.replace("{pet_xp_h}", String.valueOf(premium.getBonusPetExpRate()));
				html.replace("{raid_drop_chance_h}", String.valueOf(premium.getBonusRaidDropChance()));
				html.replace("{raid_drop_amount_h}", String.valueOf(premium.getBonusRaidDropAmount()));
				html.replace("{herb_drop_chance_h}", String.valueOf(premium.getBonusHerbDropChance()));
				html.replace("{herb_drop_amount_h}", String.valueOf(premium.getBonusHerbDropAmount()));
