...
import l2r.gameserver.utils.BbsUtil;
...
//		html = BbsUtil.htmlAll(html, player);
	...
	private static String getBossListReplacements(String html, int page, Map<Integer, StatsSet> allBosses, boolean nextPage)
	{
		...
		if (page > 0)
			builder.append("<td width=380 align=right><button value=\"Prev\" action=\"bypass _bbsbosslist_%sort%_").append(page - 1).append("_%search%\" width=120 height=22 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.button_df\"></td>");
		else
			builder.append("<td width=380 align=right><br></td>");
		// Next
		if (nextPage)
			builder.append("<td width=380 align=left><button value=\"Next\" action=\"bypass _bbsbosslist_%sort%_").append(page + 1).append("_%search%\" width=120 height=22 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.button_df\"></td>");
		else
			builder.append("<td width=380 align=right><br></td>");
...
//					player.sendMessage("Boss isn't alive!");
	...
	private static String getDetailedBossReplacements(Player player, String html, StatsSet bossSet, NpcTemplate bossTemplate, NpcInstance boss)
	{
		...
		newHtml = newHtml.replace("%maxHp%", String.valueOf((int) bossTemplate.getBaseHpMax()));
		...
		newHtml = newHtml.replace("%maxMp%", String.valueOf((int) bossTemplate.getBaseMpMax()));
		...
		// Combat:
		newHtml = newHtml.replace("%atkspeed%", String.valueOf(bossTemplate.getBasePAtkSpd())); // atk speed
		newHtml = newHtml.replace("%cspeed%", String.valueOf(bossTemplate.getBaseMAtkSpd())); // casting speed
		newHtml = newHtml.replace("%speed%", String.valueOf(bossTemplate.getBaseRunSpd())); // run speed
		newHtml = newHtml.replace("%pAtk%", String.valueOf(bossTemplate.getBasePAtk())); // p atk
		newHtml = newHtml.replace("%pDef%", String.valueOf(bossTemplate.getBasePDef())); // p def
		newHtml = newHtml.replace("%mAtk%", String.valueOf(bossTemplate.getBaseMAtk())); // m atk
		newHtml = newHtml.replace("%mDef%", String.valueOf(bossTemplate.getBaseMDef())); // m def
		newHtml = newHtml.replace("%accuracy%", String.valueOf(bossTemplate.getBaseShldRate())); // maybe accuracy?
...
//		newHtml = newHtml.replace("%MEN%", String.valueOf(bossTemplate.getBaseMEN()));
		...
		newHtml = newHtml.replace("%holy%", String.valueOf(bossTemplate.getBaseAttributeDefence()[2]));
		newHtml = newHtml.replace("%water%", String.valueOf(bossTemplate.getBaseAttributeDefence()[3]));
		newHtml = newHtml.replace("%earth%", String.valueOf(bossTemplate.getBaseAttributeDefence()[4]));
		newHtml = newHtml.replace("%dark%", String.valueOf(bossTemplate.getBaseAttributeDefence()[5]));
