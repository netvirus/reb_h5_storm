...
public class CommunityBoardCareerManager {
    ...
    public void onBypassCommand(Player activeChar, String command)
    {
        ...
        else if(command.startsWith("_bbscareer;sub;"))
        {
            if (!activeChar.isGM())
            {
        ...
        else if(command.equalsIgnoreCase("_bbscareer;certification;"))
        {
    ...
    private static boolean CheckCondition(Player player)
    {
        ...
        if (!player.isInZone(Zone.ZoneType.peace_zone) && !player.isInZone(Zone.ZoneType.RESIDENCE) || NexusEvents.isRegistered(player) || NexusEvents.isInEvent(player) || player.isInOlympiadMode()) {
    ...
    public static final CommunityBoardCareerManager getInstance()
    {
        return CommunityBoardCareerManager.SingletonHolder._instance;
    }

    private static class SingletonHolder
    {
        protected static final CommunityBoardCareerManager _instance = new CommunityBoardCareerManager();
    }
}
