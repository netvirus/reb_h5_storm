	...
	protected void runImpl()
	{
		...
		try
		{
			...
			if (activeChar.canOverrideCond(PcCondOverride.DEBUG_CONDITIONS))
				activeChar.sendMessage("Bypass: " + bp.bypass);
			
			if (NexusEvents.onBypass(activeChar, bp.bypass))
				return;
			...
			if(bp.bypass.startsWith("admin_"))
			{
				...
				if(NexusEvents.adminCommandRequiresConfirm(bp.bypass.split(" ")[0]))
				{
			...
			{
				String p = bp.bypass.substring(10).trim();
				OfflineBufferManager.getInstance().processBypass(activeChar, p);
			}
		...
		catch(Exception e)
		{
			...
			if(target != null && target.isNpc())
				st = st + " via NPC #" + ((NpcInstance) target).getNpcId() + " Requested by: " + activeChar.getName();
			_log.error(st, e);
		}
	}
