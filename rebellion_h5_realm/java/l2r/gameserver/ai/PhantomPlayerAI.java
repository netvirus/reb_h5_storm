...
import l2r.gameserver.ai.DefaultAI.Task;
import l2r.gameserver.ai.DefaultAI.TaskType;
import l2r.gameserver.geodata.GeoEngine;
import l2r.gameserver.instancemanager.ReflectionManager;
import l2r.gameserver.listener.actor.OnKillListener;
import l2r.gameserver.listener.actor.OnMagicHitListener;
import l2r.gameserver.listener.actor.ai.OnAiEventListener;
...
import l2r.gameserver.model.instances.WarehouseInstance;
...
import l2r.gameserver.randoms.LocationStorage;
...
public class PhantomPlayerAI extends PlayerAI implements OnAiEventListener, OnMagicHitListener, OnKillListener
{
	...
	public void runImpl() throws Exception
	{
		...
		if (Config.DISABLE_PHANTOM_ACTIONS)
			return;
	...
	{
		_l1: for (int copyObjId : LocationStorage.getLocationsKeys())
		{
	...
	protected boolean thinkActive()
	{
		try
		{
			...
			if (actor.getCurrentHpPercents() < 70)
			{
				...
				{
					int skillTime = skill.isSkillTimePermanent() ? skill.getHitTime() : Formulas.calcMAtkSpd(getActor(), skill, skill.getHitTime());
			...
			if (actor.getClassId().isMage())
			{
				...
				{
					int timeToWaitInMilis = skillCast == null ? reuse : Formulas.calcMAtkSpd(getActor(), skillCast, skillCast.getHitTime()) + reuse;
	...
	protected Creature getTarget()
	{
		try
		{
			...
			final List<Creature> list = World.getAroundCharacters(getActor()).stream()
				.filter(Objects::nonNull)
				.filter(t -> !t.isRaid() && !t.isDead() && !t.isMinion() && !(t instanceof GuardInstance) && !t.isInvisible() && !t.isInvul() && t.isMonster() && !(t instanceof TreasureChestInstance))
			...
			OUT: for (Creature tgt : list)
			{
				...
				{
					if (Location.getDistance(getActor(), tgt) >= 2000) // CheckTarget failed due to distance.
						lowPriorityTargets.add(tgt);
	...
	public void startRoamingInTown()
	{
		...
		setMood(Rnd.get(new String[]{ "sit and relax", "free roam", "check npcs", "check shop", "check warehouse"}));
	...
	private boolean isInTown()
	{
		...
		for (Zone zone : getActor().getZones())
		{
			if (zone.getName().contains("talking_island_town_peace_zone") ||
				zone.getName().contains("darkelf_town_peace_zone") ||
				zone.getName().contains("elf_town_peace") ||
				zone.getName().contains("guldiocastle_town_peace") ||
				zone.getName().contains("gludin_town_peace") ||
				zone.getName().contains("dion_town_peace") ||
				zone.getName().contains("floran_town_peace") ||
				zone.getName().contains("giran_town_peace") ||
				zone.getName().contains("orc_town_peace") ||
				zone.getName().contains("dwarf_town_peace") ||
				zone.getName().contains("oren_town_peace") ||
				zone.getName().contains("hunter_town_peace") ||
				zone.getName().contains("aden_town_peace") ||
				zone.getName().contains("speaking_port_peace") ||
				zone.getName().contains("gludin_port") ||
				zone.getName().contains("giran_port") ||
				zone.getName().contains("heiness_peace") ||
				zone.getName().contains("godad_peace") ||
				zone.getName().contains("rune_peace") ||
				zone.getName().contains("gludio_airship_peace") ||
				zone.getName().contains("schuttgart_town_peace") ||
				zone.getName().contains("kamael_village_town_peace") ||
				zone.getName().contains("keucereus_alliance_base_town_peace") ||
				zone.getName().contains("giran_harbor_peace_alt") ||
			...
			try
			{
				...
				if (ai.isInTown())
				{
					...
					for (Entry<Integer, Location> entry : HUNTING_GROUNDS.entrySet())
					{
						...
						if ((_player.getLevel() >= 75 && minLevel >= 75) || (_player.getLevel() + maxLevelDiff > minLevel && _player.getLevel() < minLevel))
							locs.add(loc);
					}
		...
		switch (soeType)
		{
			...
			default:
				if (getActor().getLevel() > 75)
					soe = Rnd.get(new Skill[]{SOE_ADEN_CASTLE_TOWN, SOE_GLUDIN_VILLAGE, SOE_HUNTERS_VILLAGE, SOE_RUNE_TOWNSHIP, SOE_TOWN_OF_GIRAN, SOE_TOWN_OF_GODDARD, SOE_TOWN_OF_OREN});
				else if (getActor().getLevel() > 60)
					soe = Rnd.get(new Skill[]{SOE_ADEN_CASTLE_TOWN, SOE_HUNTERS_VILLAGE, SOE_RUNE_TOWNSHIP, SOE_TOWN_OF_GIRAN, SOE_TOWN_OF_GODDARD, SOE_TOWN_OF_OREN, SOE_TOWN_OF_SCHUTTGART});
				else if (getActor().getLevel() > 40)
					soe = Rnd.get(new Skill[]{SOE_ADEN_CASTLE_TOWN, SOE_HUNTERS_VILLAGE, SOE_TOWN_OF_GIRAN, SOE_TOWN_OF_OREN, SOE_TOWN_OF_SCHUTTGART, SOE_TOWN_OF_DION, SOE_HEINE});
				else if (getActor().getLevel() > 20)
					soe = Rnd.get(new Skill[]{SOE_HUNTERS_VILLAGE, SOE_TOWN_OF_GIRAN, SOE_TOWN_OF_DION, SOE_HEINE, SOE_TOWN_OF_GLUDIO, SOE_GLUDIN_VILLAGE});
				else
				{
	...
	public void buffFromNpcBuffer(int curTries)
	{
		...
		Util.communityNextPage(getActor(), "_bbsbuff;buffmyclass");
		Util.communityNextPage(getActor(), "_bbsbuff;noblesse");
		Util.communityNextPage(getActor(), "_bbsbuff;heal");
	...
	protected void upgradeClass()
	{
		if (getActor().getClassId().getLevel() < 4 && // Not 3rd class
			((getActor().getLevel() >= 20 && getActor().getClassId().getLevel() == 1) ||
			(getActor().getLevel() >= 40 && getActor().getClassId().getLevel() == 2) ||
	...
	public void onMagicHit(Creature actor, Skill skill, Creature caster)
	{
		if (actor == caster)
		{
			...
			if (skill == SOE_BLESSED ||
				skill == SOE_CLANHALL ||
				skill == SOE_CASTLE ||
				skill == SOE_BLESSED_CLANHALL ||
				skill == SOE_BLESSED_CASTLE ||
				skill == SOE ||
				skill == SOE_TALKING_ISLAND_VILLAGE ||
				skill == SOE_ELVEN_VILLAGE ||
				skill == SOE_DARK_ELF_VILLAGE ||
				skill == SOE_ORC_VILLAGE ||
				skill == SOE_DWARVEN_VILLAGE ||
				skill == SOE_GLUDIN_VILLAGE ||
				skill == SOE_TOWN_OF_GLUDIO ||
				skill == SOE_TOWN_OF_DION ||
				skill == SOE_FLORAN_VILLAGE ||
				skill == SOE_TOWN_OF_GIRAN ||
				skill == SOE_HARDIN_ACADEMY ||
				skill == SOE_HEINE ||
				skill == SOE_TOWN_OF_OREN ||
				skill == SOE_IVORY_TOWER ||
				skill == SOE_HUNTERS_VILLAGE ||
				skill == SOE_HUNTERS_VILLAGE ||
				skill == SOE_TOWN_OF_GODDARD ||
				skill == SOE_RUNE_TOWNSHIP ||
				skill == SOE_TOWN_OF_SCHUTTGART ||
				skill == SOE_KETRA_ORC_VILLAGE ||
				skill == SOE_VARKA_SILENOS_VILLAGE ||
				skill == SOE_KAMAEL_VILLAGE)
			{
