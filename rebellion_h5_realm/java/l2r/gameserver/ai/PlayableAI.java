...
import l2r.gameserver.model.Skill.NextAction;
...
public class PlayableAI extends CharacterAI
{
	...
	protected void thinkFollow()
	{
		...
		try
		{
			...
			if(target == null || !target.isVisible() || target.isAlikeDead() || actor.getDistance(target) > 4000 || offset == null)
			{
				clientActionFailed();
				return;
			}
	...
	protected void thinkCast(boolean checkRange)
	{
		...
		if(!checkRange)
		{
			//if (!_forceUse && _skill.getNextAction() == NextAction.ATTACK && target.isAutoAttackable(actor))
		...
		canSee &= !(_skill.getFlyType() == FlyType.DUMMY || _skill.getFlyType() == FlyType.CHARGE) || actor.getFlyLocation(target, _skill) != null;
	...
	public void Cast(Skill skill, Creature target, boolean forceUse, boolean dontMove)
	{
		...
		if (actor.isPlayer() && actor.getCastingSkill() != null && actor.getCastingSkill().getSkillType() == SkillType.TRANSFORMATION)
		{
			clientActionFailed();
			return;
		}
