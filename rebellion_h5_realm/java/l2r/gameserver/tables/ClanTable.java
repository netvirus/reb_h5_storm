...
public class ClanTable
{
	...
	public void restoreClans()
	{
		...
		for (int clanId : clanIds)
		{
			...
			{
				_log.info("ClanDissolve: clanId=" + clan.getClanId() + " dissolvedClanTime=" + TimeUtils.convertDateToString(clan.getDissolvedClanTime()) + " systemTime=" + TimeUtils.convertDateToString(System.currentTimeMillis()));
	...
	public void startClanWar(Clan clan1, Clan clan2)
	{
		...
		CharacterNameColorization.updateColor(clan1.getOnlineMembers(0).toArray(new Player[0]));
		CharacterNameColorization.updateColor(clan2.getOnlineMembers(0).toArray(new Player[0]));
	}
	...
	public void destroyClan(int clanId)
	{
		...
		for (Player clanMember : clan.getOnlineMembers(0))
		{
			...
			clanMember.setClan(null);
			clanMember.setTitle(null);
			clanMember.sendPacket(new IStaticPacket[] { PledgeShowMemberListDeleteAll.STATIC, SystemMsg.YOU_HAVE_RECENTLY_BEEN_DISMISSED_FROM_A_CLAN });
			clanMember.setLeaveClanTime(curtime);
