...
public class Scripts
{
	...
	private void load()
	{
		...
		if (Config.DONTLOADQUEST)
			_log.info("Scripts: Skipping quests.");
		if (Config.DONTLOADEVENTS)
			_log.info("Scripts: Skipping events.");
	...
	public void init()
	{
		for (Class<?> clazz : _classes.values())
		{
			...
			if (Config.DONTLOADQUEST && ClassUtils.isAssignable(clazz, Quest.class))
				continue;
			if (Config.DONTLOADEVENTS && clazz.getName().startsWith("events")) // If package starts with events
				continue;
	...
	/**
	 * Load all scripts from data/scripts/<b>target</b>.
	 * If <b>target</b> is an empty string, it will load all scripts.
	 * @param classes add to what list of classes.
	 * @param target dir to script folder or script file.
	 * @return true, false if an error occured.
	 */
	private boolean load(List<Class<?>> classes, String target)
	{
		...
		else
		{
			...
			IOFileFilter filter = new DirectoryFileFilter()
			{
			    ...
			    {
					if (Config.DONTLOADQUEST && file.isDirectory() && file.getPath().contains("quests/"))
						return false;
					if (Config.DONTLOADEVENTS && file.isDirectory() && file.getPath().contains("events/")) // If package starts with events
						return false;
					
			        return file.isDirectory();
			    }
			};
			scriptFiles = FileUtils.listFiles(file, FileFilterUtils.suffixFileFilter(".java"), filter);
		}
