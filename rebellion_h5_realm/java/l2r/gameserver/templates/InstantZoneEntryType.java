...
public enum InstantZoneEntryType
{
	...
	PARTY
	{
		...
		public boolean canEnter(Player player, InstantZone instancedZone)
		{
			...
			if (party.size() < instancedZone.getMinParty())
			{
			...
			for (Player member : grp)
			{
				...
				if (member.isDualbox(Config.INSTANCES_MAX_BOXES, true, party))
				{
					if (Config.INSTANCES_MAX_BOXES == 1)
						party.sendMessage(member.getName() + " is not allowed to enter the instance because he controls more than " + (Config.INSTANCES_MAX_BOXES + 1) + (Config.INSTANCES_MAX_BOXES == 1 ? " characters in the party." : " character in the party."));
					else
						party.sendMessage(member.getName() + " is not allowed to enter the instance because he controls more than " + (Config.INSTANCES_MAX_BOXES + 1) + (Config.INSTANCES_MAX_BOXES == 1 ? " characters in the party." : " character in the party."));
	...
	COMMAND_CHANNEL
	{
		...
		public boolean canEnter(Player player, InstantZone instancedZone)
		{
			...
			for (Player member : cc)
			{
				...
				if (member.isDualbox(Config.INSTANCES_MAX_BOXES, true, cc))
				{
					if (Config.INSTANCES_MAX_BOXES == 1)
						cc.sendMessage(member.getName() + " is not allowed to enter the instance because he controls more than " + (Config.INSTANCES_MAX_BOXES + 1) + (Config.INSTANCES_MAX_BOXES == 1 ? " characters in the command channel." : " character in the command channel."));
					else
						cc.sendMessage(member.getName() + " is not allowed to enter the instance because he controls more than " + (Config.INSTANCES_MAX_BOXES + 1) + (Config.INSTANCES_MAX_BOXES == 1 ? " characters in the command channel." : " character in the command channel."));
	...
	private static SystemMsg checkPlayer(Player player, InstantZone instancedZone)
	{
		...
		// TODO проверку на итемы с ID меньше 0
