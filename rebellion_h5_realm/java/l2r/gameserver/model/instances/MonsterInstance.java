...
import l2r.gameserver.model.Manor;
import l2r.gameserver.model.MinionList;
...
public class MonsterInstance extends NpcInstance
{
	...
	private RewardItemResult _harvestItem;
	...
	private List<RewardItemResult> _sweepItems;
	...
	protected void onDeath(Creature killer)
	{
		...
		if(Config.ENABLE_PLAYER_COUNTERS && killer != null && killer != this && killer.getPlayer() != null)
		{
			if (getChampionTemplate() != null)
				killer.getPlayer().getCounters().addPoint("_Champions_Killed");
			else
				killer.getPlayer().getCounters().addPoint("_Monsters_Killed");
		}
	...
	public RewardItemResult takeHarvest()
	{
		harvestLock.lock();
		try
		{
			RewardItemResult harvest;
	...
	public boolean setSeeded(Player player, int seedId, boolean altSeed)
	{
		...
		try
		{
			...
			_harvestItem = new RewardItemResult(Manor.getInstance().getCropType(seedId));
	...
	public List<RewardItemResult> takeSweep()
	{
		sweepLock.lock();
		try
		{
			List<RewardItemResult> sweep = _sweepItems;
			clearSweep();
			return sweep;
		}
		finally
	...
	private void checkUD(Creature attacker, double damage)
	{
		...
		if(isMinion() || getMinionList() != null || isRaid() || this instanceof ReflectionBossInstance || this instanceof TreasureChestInstance || getChampion() > 0)
			return;
