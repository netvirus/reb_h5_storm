...
public class NpcInstance extends Creature
{
		...
		if (getChampionTemplate() != null)
		{
			for (ChampionTemplateTable.RewardItem ri : getChampionTemplate().rewards)
			{
				if (Rnd.get(100) < ri.getDropChance()) // We dont use <= since the random gets one from 0 to 99
				{
					...
					item = ItemFunctions.createItem(ri.getItemId());
					item.setCount(count);
	...
	{
		if (attacker != null && attacker.isPlayer() && attacker.getPlayer().isInPeaceZone())
			return Config.NPC_ALLOW_HIT;
	...
	{
		if (getChampionTemplate() != null) 
			return (int) (super.getMAtk(target, skill) * Config.ALT_NPC_MATK_MODIFIER * getChampionTemplate().matkMultiplier);
	...
	{
		if (getChampionTemplate() != null) 
			return (int) (super.getPAtk(target) * Config.ALT_NPC_PATK_MODIFIER * getChampionTemplate().patkMultiplier);
	...
	{
		if (getChampionTemplate() != null) 
			return (int) (super.getMaxHp() * Config.ALT_NPC_MAXMP_MODIFIER * getChampionTemplate().hpMultiplier);
