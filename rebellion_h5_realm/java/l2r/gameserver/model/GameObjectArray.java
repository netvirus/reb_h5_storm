...
public class GameObjectArray<E extends GameObject> implements Iterable<E>
{
	...
	public synchronized int add(E e)
	{
		...
		if(elementData.length <= size)
		{
			...
			if (e.isNpc()) // Only for NPCs
			{
				TreeMap<Integer, Integer> npcs = new TreeMap<Integer, Integer>(); // Id, count
				for (GameObject npc : elementData)
				{			
					Integer curCount = npcs.get(((Creature)npc).getNpcId());
					if (curCount == null)
						npcs.put(((Creature)npc).getNpcId(), 1);
					else
						npcs.put(((Creature)npc).getNpcId(), curCount+1);
				}
				
				List<Map.Entry<Integer, Integer>> list = new ArrayList<Map.Entry<Integer, Integer>>(npcs.entrySet());
		        ...
		        for (int i = 0; i < Math.min(10, list.size()); i++) // Log top 10 consumers.
		        {
		        	Map.Entry<Integer, Integer> entry = list.get(i);
		        	int npcId = entry.getKey();
		        	int spawnedCount = entry.getValue();
		        	_log.warn("Npc: " + NpcHolder.getNpcName(npcId) + "(" + npcId + ") has a current spawn count of " + spawnedCount);
		        }
			}
			Thread.dumpStack();
			_log.warn("Object array [" + name + "] resized info end <----------------------");
	...
	{
		return Spliterators.spliterator(elementData, 0);
	}
