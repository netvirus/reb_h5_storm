...
import l2r.gameserver.listener.actor.door.impl.MasterOnOpenCloseListenerImpl;
import l2r.gameserver.listener.reflection.OnReflectionCollapseListener;
import l2r.gameserver.listener.zone.impl.AirshipControllerZoneListener;
import l2r.gameserver.listener.zone.impl.DominionWardEnterLeaveListenerImpl;
import l2r.gameserver.listener.zone.impl.DuelZoneEnterLeaveListenerImpl;
import l2r.gameserver.listener.zone.impl.NoLandingZoneListener;
import l2r.gameserver.listener.zone.impl.ResidenceEnterLeaveListenerImpl;
...
import l2r.gameserver.templates.InstantZone.ZoneInInstanceInfo;
...
public class Reflection
{
	...
	private boolean _allowSummon = false;
	...
	public void addDoor(int doorId, Reflection ref, boolean opened)
	{
		if (_doors == null || _doors.isEmpty())
			_doors = new HashIntObjectMap<DoorInstance>(1);
		...
		DoorTemplate doortemplate = DoorHolder.getInstance().getTemplate(doorId);
		
		if (doortemplate == null)
			return;
		
		DoorInstance newdoor = new DoorInstance(IdFactory.getInstance().getNextId(), doortemplate);
		newdoor.setReflection(ref);
		newdoor.setIsInvul(true);
		newdoor.spawnMe(doortemplate.getLoc());
		...
		_doors.put(doortemplate.getNpcId(), newdoor);
	...
	/**
	 * Used to init reflections using ZoneInInstanceInfo.
	 * @param zones - <String, ZoneInInstanceInfo>
	 * @param doors
	 */
	private void init0(Map<String, ZoneInInstanceInfo> zones, IntObjectMap<InstantZone.DoorInfo> doors)
	{
		if(!doors.isEmpty())
			_doors = new HashIntObjectMap<DoorInstance>(doors.size());
		...
		for(ZoneInInstanceInfo t : zones.values())
		{
	...
	public NpcInstance addSpawnWithoutRespawn(int npcId, Location loc, int randomOffset)
	{
		Location newLoc;
		if(randomOffset > 0)
			newLoc = loc.findPointToStay(randomOffset);
		else
			newLoc = loc;

		return SpawnTable.spawnSingle(npcId, newLoc, this);
	}
	...
	public NpcInstance addSpawnWithRespawn(int npcId, Location loc, int randomOffset, int respawnDelay)
	{
		...
		sp.setLoc(randomOffset > 0 ? loc.findPointToStay(randomOffset) : loc);
	...
	public void init(InstantZone instantZone)
	{
		...
		init0(instantZone.getZones(), instantZone.getDoors());
