...
public final class QuestState
{
                ...
                case Quest.PARTY_ALL:
                    ...
                    else
                    {
                        ...
                        for(Player $member : actorPlayer.getParty())
                            if($member.isInRange(actorPlayer, Config.ALT_PARTY_DISTRIBUTION_RANGE))
                                players.add($member);
                    }
                    break;
                default:
    ...
    public void addExpAndSp(long exp, long sp)
    {
        ...
        double rate = getRateQuestsRewardExpSp();

        if (Config.ALLOW_QUETS_ADDONS_CONFIG)
        {
            double addonrate = Config.getQuestDropRates(getQuest());
            if (addonrate > 0)
                rate = addonrate;
        }

        if(exp > 0)
            player.addExpAndSp((long) (exp * rate), 0);

        if(sp > 0)
            player.addExpAndSp(0, (long) (sp * rate));
    }
    ...
    public void addRadarWithMap(int x, int y, int z)
    {
        Player player = getPlayer();
        if(player != null)
            player.addRadarWithMap(x, y, z, true);
    }
    ...
    public void giveItems(int itemId, long count, boolean rate)
    {
        ...
        if(rate)
        {
            if (itemId != ItemTemplate.ITEM_ID_ADENA)
                ...
                count = (long) (count * Config.QUEST_REWARD_ADENA);
        }

        if(itemId == 57)
            Log.addGame("Quest|" + getQuest().getQuestIntId() + "|" + count + "|" + player.getName(), "adena");
    ...
    public double getRateQuestsDrop()
    {
        ...
        if (Config.ALLOW_QUETS_ADDONS_CONFIG)
        {
            double rate = Config.getQuestDropRates(getQuest());
            if (rate > 0)
                return rate;
            else
    ...
    public double getRateQuestsReward()
    {
        ...
        if (Config.ALLOW_QUETS_ADDONS_CONFIG)
        {
            double rate = Config.getQuestRewardRates(getQuest());
            if (rate > 0)
                return rate;
            else
