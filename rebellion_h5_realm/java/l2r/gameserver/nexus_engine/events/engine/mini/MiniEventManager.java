...
import l2r.gameserver.nexus_engine.events.engine.mini.features.ItemsFeature;
import l2r.gameserver.nexus_engine.events.engine.mini.features.StrenghtChecksFeature;
...
{
	protected static final Logger _log = LoggerFactory.getLogger(MiniEventManager.class);
	...
	private FastTable<String> _configCategories;
	...
	protected Comparator<RegistrationData> _compareByGearScore = new Comparator<RegistrationData>()
	{
		...
		public int compare(RegistrationData p1,
						   RegistrationData p2)
		{
			double level1 = p1.getAverageGearScore();
			double level2 = p2.getAverageGearScore();
	...
	public MiniEventManager(EventType type)
	{
		...
		_parties = new FastTable<RegistrationData>();
		_games = new FastTable<MiniEventGame>();
		...
		_configCategories = new FastTable<String>();
	...
	public void loadConfigs()
	{
		...
		addConfig(new ConfigModel("notAllowedSkills", "", "Put here skills that won't be available for use in this event <font color=7f7f7f>(write one skill's ID and click Add, to remove the skill, simply click on it's ID in the list)</font>", InputType.MultiAdd));
		addConfig(new ConfigModel("notAllowedItems", "", "Put here items that won't be available for use in this event <font color=7f7f7f>(write one skill's ID and click Add; to remove the skill, simply click on it's ID in the list)</font>", InputType.MultiAdd));
		...
		addConfig(new ConfigModel("dualboxCheckForEnemiesIP", "true", "On matchmaking it will check the enemy IP if its equals will search for another one. (If you disable this it will use HWID check.)", InputType.Boolean));
		addConfig(new ConfigModel("dualboxCheckForEnemiesHwid", "true", "Hwid check for mini events when searching for enemy. (You have to disable dualboxCheckForEnemiesIP to use this method.)", InputType.Boolean));
	...
	protected boolean checkItems(PlayerEventInfo player)
	{
		...
		/*if(isNewbieOnly())
		{
			player.sendMessage("This is a Newbie-event - all items above S80 grade are not permitted.");
			for(L2ItemInstance item : player.getOwner().getInventory().getItems())
			{
				if(item != null && item.isEquipped())
				{
					if(item.isARestrictedItemOrcArea())
					{
						player.sendMessage("Item " + item.getItemName() + " is not allowed on this event. Unequip it before registering.");
						allow = false;
					}
				}
			}
	...
	public boolean canFight(RegistrationData data)
	{
		...
		if(requireParty())
		{
			...
			if(data.getParty().size() > getDefaultPartySizeToJoin())
			{
	...
	{
	    if (getBoolean("dualboxCheckForEnemiesIP"))
	    {
	...
	public final void notifyGameEnd(MiniEventGame game, RegistrationData winner, EventMap map)
	{
	...
	public int getRegisteredTeamsCount()
	{
		if(isTournamentActive())
		{
			return TournamentManager.getTournament().getRegisteredSize();
		}
